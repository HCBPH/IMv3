...
//        log.warn("filter");
        ...
        if (msg instanceof ByteBuf){
            ...
            if(!Comparator.compare(identifier, IMConfiguration.SEPARATOR)){
                Log.i("FilterHandler", "非IM消息,使用html解析");
            }else{
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        Log.e("FilterHandler", cause.getMessage());
    }

    @Override
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        Log.i("FilterHandler", ctx.channel().remoteAddress()+"建立连接");
        super.channelActive(ctx);
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        Log.i("FilterHandler", ctx.channel().remoteAddress()+"断开连接");
