...
public interface GroupMapper {
    ...
    @Select("select id,name,creator_id,create_time,type,group_profile from "+table_name+" where id=#{gid} and state=1")
    public Group findGroup(@Param("gid") Long gid);

    @Select("select id,name,creator_id,create_time,type,group_profile from "+table_name+" where state=1")
